...
- 54 of the 60 real Standard generals (15 per kingdom — verified directly against the upstream C++ source) and their skills — only 6 remain, each individually analyzed against the real upstream source and genuinely blocked on a specific new subsystem this engine doesn't have yet (Nullification/anti-trick counter-play, Iron Chain, Fire Attack, an Armor equip category, a private-pile dying-immunity mechanic, discard-to-skip-a-phase, multi-target Slash, re-hiding an already-revealed general) — see `webport/README.md`'s Milestone 24/25 sections for the exact list and reasoning per general. 2 real pre-existing engine bugs (Leiji crediting the wrong player, a viewAs-Indulgence card vanishing without a trace) were also found and fixed along the way, both now regression-tested.
