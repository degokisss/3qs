...
- All 60 real Standard generals (15 per kingdom — verified directly against the upstream C++ source) and their portable skills — the full roster is ported; a small number of individual sibling skills across the roster remain deferred where they need a subsystem this engine genuinely doesn't have (gender/face-up-down state, equip-stealing, ignoring a target's armor specifically, or a re-hiding action this port's own reveal-timing simplification makes unobservable) — see `webport/README.md`'s Milestone log for the exact per-general reasoning. Plus 19 generals from outside the Standard set (Identity mode never offers these -- see Milestone 39): the ENTIRE assessed roster of upstream's `momentum.cpp`/`formation.cpp` packages (13 generals, see Milestone 42), plus 6 out of `transformation.cpp` (Milestone 43-45: Xun You, Li Guo, Ma Su, Ling Tong, Bian Huanghou, Sha Moke — the last 2 unlocked by 2 new engine subsystems: a hook that lets a skill pay a card to cancel itself as a Dismantlement/Snatch target, and a multi-target-Slash framework since this engine originally hardcoded Slash to exactly 1 target) -- the rest of that package (Zuo Ci, Lu Fan) stays deferred, headlined by Zuo Ci's Huashen, a whole-general skill-ACQUISITION mechanic this engine genuinely doesn't have. Most ported generals here are partial (1-2 of their real 2-3 skills, e.g. Zang Ba, Li Dian, Jiang Wei), a few are complete kits (Ma Dai including a real card-USE-restriction subsystem for Tiềm Tập -- Milestone 40; He Taihou; Ma Su) — every deferred skill/general has a specific documented reason, and `jiange-defense.cpp` (a separate scenario mode, not draftable Quốc Chiến generals at all) stays deferred entirely.
- Hegemony/Quốc Chiến mode: kingdom-team setup + the official Ambitionist overflow rule + faction-based ally/win-condition logic + the real dual-general (主将/副將) system (each player drafts 2 same-kingdom generals, HP/skills/gender combine per the official rule) + the face-down/face-up (暗置/明置) reveal-TIMING mechanic (both generals start hidden — real fog-of-war, not just a client-side toggle — until each player chooses to reveal, which is also when their kingdom/Ambitionist status is finally decided, AND when their skills actually become usable — a hidden general's abilities are genuinely inert, not just visually hidden) + "Ao Chiến" (鏖战, once the table narrows to ≤4 distinct factions Peach stops healing and instead becomes playable as Slash or Jink) + 珠联璧合 companion-pair bonuses (a recover-or-draw choice, verified against the real upstream C++ source for the 22 companion pairs where both generals are actually ported here) + the leftover-half-HP bonus draw, both offered at the real trigger moment (the instant a player's 2nd general reveals), selectable per room from the lobby. Not built (checked directly against the real upstream C++ source, not just this repo's own roster): formation skills (阵法技/围攻/队列, a genuinely separate package of unported generals) and kingdom-wide lord-skill team bonuses (e.g. `shouyue`).
