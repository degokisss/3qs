...
- 57 of the 60 real Standard generals (15 per kingdom — verified directly against the upstream C++ source) and their skills — only 3 remain, each individually analyzed against the real upstream source and genuinely blocked on a specific new subsystem this engine doesn't have yet (Nullification/anti-trick counter-play, Iron Chain, Fire Attack, an Armor equip category, a private-pile dying-immunity mechanic, re-hiding an already-revealed general, a reveal-ask-timing architecture change) — see `webport/README.md`'s Milestone 24/25/26 sections for the exact list and reasoning per general. 3 real pre-existing engine bugs (Leiji crediting the wrong player, a viewAs-Indulgence card vanishing without a trace, a mid-resolution draw-pile reshuffle silently orphaning an in-flight discard-pile reference) were also found and fixed along the way, all now regression-tested.
