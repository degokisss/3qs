...
- 49 of the 60 real Standard generals (15 per kingdom — verified directly against the upstream C++ source) and their skills. The remaining 11 are individually analyzed against the real upstream source: each needs a genuinely new subsystem this engine doesn't have yet (Nullification/anti-trick counter-play, Iron Chain, Fire Attack, an Armor equip category, a private-pile dying-immunity mechanic, discard-to-skip-a-phase, multi-target Slash, re-hiding an already-revealed general, a free virtual-card Slash, a per-target distance override, a reactive 3rd-party equip trigger) — see `webport/README.md`'s Milestone 24 section for the exact list and reasoning per general.
